...
- `stash` of a hunk in an intent-to-add entry (`git add -N`), or in a rename whose new side is one, took the file out of the worktree and left the entry naming it, so it showed as deleted. It is now refused, as `git stash` refuses it, and nothing changes: `error: cannot stash intent-to-add entry '<path>'`, exit 1. Hunks in other files still stash, with the entry left as it is.
