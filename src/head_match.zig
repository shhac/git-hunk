//! Mapping index-relative hunks onto HEAD-relative ones.
//!
//! A stash records its tracked changes as a tree built from HEAD, but the
//! user selects hunks from the index↔worktree diff. When the index is dirty
//! the two diffs disagree about hunk boundaries, so each selected index hunk
//! is matched to the HEAD hunks it overlaps by worktree line position, and a
//! partial overlap becomes a line selection. Pure: no git, no I/O.
